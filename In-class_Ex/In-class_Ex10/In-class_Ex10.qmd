...
# In-Class Ex 10: Spatial Interaction Models

**(With Notes)**

Spatial interaction = flow from one location to another location.

# A Geocoding

postcode \<- unique(condo_resale\$`Postal Code`) \<- only pick the unique post code, then do geocoding

# B Review Hands-on Ex
...
pacman::p_load(tmap, sf, sp,
               performance, reshape2, # reshape2 = grandfather of dplyr
               ggpubr, tidyverse)
```
...
# fii = zone flow from zone i to i
```
...
tm_shape(mpsz) +
  tm_polygons() +
flowLine %>%  
  filter(MORNING_PEAK >= 5000) %>% # 5000 <- trial and error
tm_shape() +
  tm_lines(lwd = "MORNING_PEAK",
           style = "quantile",
           scale = c(0.1, 1, 3, 5, 7, 10),
...
Jurong Area has one of the biggest flow -\> 160k flow over 4 hours interval in 5 days

# 10B: Calibrating Spatial Interaction Models

Methods usedL OLS, log-normal, Poisson and negative binomial (NB)
...
sp method is faster especially for larger dataset
...
```{r}
head(dist, n=c(10, 10)) # [1,] -> internal ID of the polygon
```
